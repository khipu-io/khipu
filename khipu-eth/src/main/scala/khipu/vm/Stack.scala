...
object Stack {
  ...
  def main(xs: Array[String]) {
    ...
    (1 to 11) foreach { n => s.push(EvmWord.safe(n)) }
    println(s)
    ...
    (1 to 11) foreach { n => s.push(Vector(EvmWord.safe(n), EvmWord.safe(-n))) }
    println(s)
  ...
  val ListOfZero = List(EvmWord.Zero)
...
final class Stack private (val maxSize: Int) {
  ...
  private var underlying: Array[EvmWord] = Array.ofDim[EvmWord](16) // initial size 16
  private var length = 0

  private def expend(toSize: Int) {
    val len = (toSize / 16 + 1) * 16
    val xs = Array.ofDim[EvmWord](len)
    System.arraycopy(underlying, 0, xs, 0, length)
    underlying = xs
  }

  def push(word: EvmWord) {
    if (length + 1 <= maxSize) {
   ...
   */
  def pop(n: Int): List[EvmWord] = {
    if (n > 0 && n <= length) {
      var res = List[EvmWord]()
      var i = n
      while (i >= 1) {
   ...
   */
  def toSeq: Seq[EvmWord] = underlying.take(length).reverse
